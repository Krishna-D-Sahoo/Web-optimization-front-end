...
// Changes the value for the size of the pizza above the slider
 ...
 function changePizzaSizes(size) {
    ...
    for(var i = 0; i < randomPizza.length; i++) {
            // console.log(randomPizza[i].offsetWidth);
            rParray[i] = randomPizza[i].offsetWidth;
            }

    var ww = document.querySelector("#randomPizzas").offsetWidth;
    pizzaWorker.postMessage([size, rParray, ww]);
    }
...
// Moves the sliding background pizzas based on scroll position
  ...
  // Debouncing the scrolling event
  var lastestScrollY = 0;
  var animator = false;

  function onScroll() {
    lastestScrollY = window.scrollY;
    requestAnimation();
  }
  ...
  function updatePositions() {
    ...
    var currentScrollY = lastestScrollY;
